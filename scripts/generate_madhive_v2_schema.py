...
TABLES = [
    ...
    dict(layer="Gold", name="agg_campaign_reach", grain="One row per campaign",
         description="Reach cannot be summed, so it is not stored as a number. What is stored is frequency -- impressions per unique identifier -- plus how much the campaigns in scope overlap. Reach for any filter is then impressions divided by frequency, less the overlap, which keeps it consistent with the impression total rather than sitting beside it.",
...
RULES = [
    ...
    dict(title="Reach is derived, never stored as a count",
         body="A stored reach number would sit beside the impression total and drift from it the moment a filter moved. Frequency is stored instead, so reach is impressions divided by frequency and stays tied to the number above it. The cross-media discount is applied on top, and the card says it is modelled rather than counted -- a device id and a hashed email are matched, not joined."),
